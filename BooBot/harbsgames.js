~createjs ~harbsgames|if(new RegExp("\\?|help|how").test(input))
...
for (var u=0; u<server.Users.length; u++) {
    user = server.Users[u];    name = user.CurrentGame;isBot = false;
    for(var z = 0; z<user.Roles.length; z++){if(user.Roles[z].Name=="Bot")isBot = true;}
    if(isBot == false)
    {
        if(name!= null)
        {
